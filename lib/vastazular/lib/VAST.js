...
VAST.prototype.resolveWrappers = function resolveWrappers(/*maxRedirects, callback*/) {
	...
	if (maxRedirects === 0) {
		return Promise.reject(new Error('Too many redirects were made.'));
	}

	return nodeifyPromise(Promise.all(this.map('wrappers', function requestVAST(wrapper) {
		return Promise.resolve(request.get(wrapper.vastAdTagURI))
		    .then(function makeVAST(response) {
		        return {
		            wrapper: wrapper,
		            response: VAST.pojoFromXML(response.data).ads
		        };
		    });
...
VAST.fetch = function fetch(uri) {
	var options = typeof arguments[1] === 'object' ? arguments[1] || {} : {};
	var callback = typeof arguments[2] === 'function' ? arguments[2] : arguments[1];

	var VAST = this;

	return nodeifyPromise(Promise.resolve(request.get(uri))
		.then(function makeVAST(response) {
			var vast = new VAST(VAST.pojoFromXML(response.data));
